...
function M.ensure_kernel(on_ready)
  local uranus = get_uranus()
  if not uranus then
  ...
  if status and status.current_kernel and status.current_kernel.name then
    if on_ready then on_ready(true) end
    return
  end

  vim.notify("No kernel connected. Please run :UranusPickKernel or select a kernel.", vim.log.levels.WARN)
  if on_ready then on_ready(false) end
end

function M.connect_kernel_by_name(kernel_name)
  local uranus = get_uranus()
  if not uranus then
    return false, "Uranus not available"
  end
  ...
  return false, result and result.error and result.error.message or "Failed to connect"
end

function M.pick_and_connect_kernel(on_connected)
  local uranus = get_uranus()
  if not uranus then
  ...
  local kernels_result = uranus.list_kernels()
  if not kernels_result or not kernels_result.success or #kernels_result.data.kernels == 0 then
    vim.notify("No kernels available. Please install ipykernel and create a kernel.", vim.log.levels.WARN)
    return
  end

  local kernels = kernels_result.data.kernels
  local kernel_names = {}
  for _, k in ipairs(kernels) do
    table.insert(kernel_names, k.name)
  end

  vim.ui.select(kernel_names, {
    prompt = "Select kernel:",
  }, function(choice)
    if choice then
      local result = uranus.connect_kernel(choice)
      if result and result.success then
        vim.notify("Connected to kernel: " .. choice, vim.log.levels.INFO)
        if on_connected then on_connected(true) end
      else
        vim.notify("Failed to connect to kernel: " .. (result and result.error and result.error.message or "Unknown error"), vim.log.levels.ERROR)
        if on_connected then on_connected(false) end
      end
    end
  end)
end
...
function M.execute_cell()
  local function do_execute()
    ...
    local status = uranus.status()
    if not status or not status.current_kernel or not status.current_kernel.name then
      if config.auto_kernel_prompt then
        vim.notify("No kernel connected. Running :UranusPickKernel...", vim.log.levels.WARN)
        M.pick_and_connect_kernel(function(connected)
          if connected then
        ...
        vim.notify("No kernel connected. Please run :UranusPickKernel", vim.log.levels.WARN)
      end
      return
    end
